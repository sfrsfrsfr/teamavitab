...
void lv_tabview_delete_tab(lv_obj_t * tabview, uint32_t idx)
{
    LV_ASSERT_OBJ(tabview, &lv_tabview_class);

    lv_tabview_t * tv = (lv_tabview_t *)tabview;
    uint32_t tab_count = lv_tabview_get_tab_count(tabview);

    if(idx >= tab_count) {
        LV_LOG_WARN("lv_tabview_delete_tab: idx %u out of range (%u tabs)",
                    (unsigned)idx, (unsigned)tab_count);
        return;
    }

    if(tab_count <= 1) {
        LV_LOG_WARN("lv_tabview_delete_tab: cannot delete the last tab");
        return;
    }

    uint32_t new_active = tv->tab_cur;

    if(tv->tab_cur == idx) {
        new_active = (idx > 0) ? idx - 1 : 0;
    }
    else if(tv->tab_cur > idx) {
        new_active = tv->tab_cur - 1;
    }

    lv_obj_t * tab_bar = lv_tabview_get_tab_bar(tabview);
    lv_obj_t * btn     = lv_obj_get_child(tab_bar, (int32_t)idx);
    if(btn) lv_obj_delete(btn);

    lv_obj_t * content = lv_tabview_get_content(tabview);
    lv_obj_t * panel   = lv_obj_get_child(content, (int32_t)idx);
    if(panel) lv_obj_delete(panel);

    tv->tab_cur = new_active;
